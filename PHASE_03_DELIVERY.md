...
cd ykay-virtual
git fetch /path/to/ykay-virtual-phase-03.bundle feature/phase-03-booking-escrow
git checkout feature/phase-03-booking-escrow        # or: git merge FETCH_HEAD
```

Run the fetch from the repo root (never from the `client/` subfolder).

Then: `docker compose up -d postgres redis` → `go run ./cmd/migrate --cmd=up`
→ `go run ./cmd/api` → `npm --prefix client run dev`.
