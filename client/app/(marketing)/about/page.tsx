      ...
      <div className="container-x py-12">
        ...
        <section className="rounded-3xl bg-brand-navy px-8 py-10 text-center text-white md:px-14">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-white/85">
            High-quality, accountable teaching beyond geography — for every
            learner, wherever they are.
          </p>
        </section>
        ...
        <section className="mt-10">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {PILLARS.map((p) => (
              <div
                key={p.title}
                className="flex flex-col gap-3 rounded-2xl border border-ink-100 bg-white p-5 shadow-soft"
              >
                <span className="grid size-11 place-items-center rounded-xl bg-brand-gold-light text-brand-green">
                  <p.icon size={20} />
                </span>
                <h3 className="font-display text-base tracking-[0.02em] text-brand-navy">
                  {p.title}
                </h3>
        ...
        <section className="mt-14 overflow-hidden rounded-3xl border border-ink-100 bg-white shadow-soft">
          <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="p-6 md:p-8">
              ...
              <h2 className="mt-2 font-display text-2xl tracking-[0.02em] text-brand-navy">
                {FOUNDER.name}
              </h2>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {FOUNDER.credentials.map((c) => (
                  <span
                    key={c}
                    className="rounded-full bg-brand-gold-light px-3 py-1.5 text-xs font-semibold text-brand-navy"
                  >
                    {c}
                  </span>
                ))}
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {FOUNDER.career.map((c) => (
                  <span
                    key={c}
                    className="rounded-full bg-ink-50 px-3 py-1.5 text-xs font-semibold text-ink-700"
                  >
                    {c}
                  </span>
                ))}
        ...
        <section className="mt-12 rounded-3xl bg-brand-gold p-10 text-center md:p-12">
          <h2 className="font-display text-2xl tracking-[0.02em] text-ink-900 md:text-3xl">
            Start learning with YK-Virtual
          </h2>
          <Link
            href="/programmes"
