...
export default function BecomeTutorSubjectsPage() {
  ...
  return (
    <main className="container-x py-12 max-w-2xl">
      <Link href="/become-tutor/apply" className="text-sm text-[#0F2A1A] font-semibold hover:underline">← Back</Link>
      <div className="mt-4">
        <OnboardingStepper current={1} />
        <SubjectsStep profileId={state.profileId} onNext={() => router.push("/become-tutor/documents")} />
      </div>
    </main>
  );
}
