    ...
    <main className="bg-surface pb-20">
      ...
      <div className="mx-auto mt-12 grid max-w-[1400px] gap-6 px-6 sm:grid-cols-2 md:px-10 lg:grid-cols-4 lg:items-center">
        {PLANS.map((p) => (
          ...
          >
            ...
            <h3 className="font-display text-xl tracking-[0.02em] text-brand-navy">{p.name}</h3>
            <span className="mt-5 font-display text-5xl tracking-[0.02em] text-ink-900">{billing === "once" ? p.priceOnce : p.priceMonthly}</span>
            <p className="mt-2 text-sm text-ink-500">{billing === "once" ? p.per : p.perMonthly}</p>
      ...
      <div className="mx-auto mt-20 max-w-[1400px] px-6 md:px-10">
        <div className="mb-10 text-center">
          <h2 className="font-display text-2xl tracking-[0.02em] text-brand-navy">Compare plans</h2>
          <p className="mx-auto mt-2 max-w-xl text-sm text-ink-500">A clear look at what each plan includes — so you pick the right one with confidence.</p>
        </div>
        ...
        <div className="hidden overflow-hidden rounded-2xl border border-ink-200 bg-white lg:block">
          <table className="w-full">
            <thead className="border-b border-ink-200 bg-white">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-medium text-ink-800">Features</th>
                {PLANS.map((p) => (
                  <th key={p.name} className="w-1/4 px-6 py-4 text-center text-lg font-medium text-ink-900">{p.name}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-ink-100">
              {COMPARE.map((row) => (
                <tr key={row.f} className="transition-colors hover:bg-ink-50/60">
                  <th className="px-6 py-5 text-left text-sm font-normal text-ink-600">{row.f}</th>
                  {row.v.map((inc, i) => (
                    <td key={i} className="px-6 py-5">
                      {inc ? (
                        <Check size={18} className="mx-auto text-brand-gold-dark" aria-label={`Included in ${PLANS[i].name}`} />
                      ) : (
                        <Minus size={18} className="mx-auto text-ink-300" aria-label={`Not included in ${PLANS[i].name}`} />
                      )}
                    </td>
                  ))}
                </tr>
          ...
          {PLANS.map((p, pi) => (
            <section key={p.name}>
              ...
              <ul className="divide-y divide-ink-100 rounded-2xl border border-ink-200 bg-white">
                {COMPARE.map((row) => (
                  <li key={row.f} className="flex items-center justify-between px-4 py-3 text-sm">
                    <span className="text-ink-600">{row.f}</span>
                    {row.v[pi] ? <Check size={16} className="text-brand-gold-dark" /> : <Minus size={16} className="text-ink-300" />}
                  </li>
                ))}
              </ul>
            </section>
      ...
      <div className="mx-auto mt-14 max-w-[1400px] px-6 md:px-10">
        <p className="text-xs text-ink-400">
          Pricing shown is indicative guidance and may vary by tutor experience, subject and level. The final quote is
          always agreed before payment. Institutional pricing for schools and companies is available via the{" "}
          <Link href="/for-schools" className="font-semibold text-brand-gold-dark">for schools</Link> and{" "}
          <Link href="/corporate-training" className="font-semibold text-brand-gold-dark">corporate training</Link> pages.
        </p>

        <section className="mt-10 rounded-2xl border border-ink-200 bg-white p-8">
          <h2 className="font-display text-xl tracking-[0.02em] text-brand-navy">Cancellation &amp; reschedule policy</h2>
        ...
        <section className="mt-10 max-w-2xl">
          <h2 className="font-display text-xl tracking-[0.02em] text-brand-navy">Frequently asked questions</h2>
          <div className="mt-4 space-y-3">
            {[
              { q: "What is included in the price?", a: "Live lessons, resources, lesson notes and progress reports. Exam packages add mocks and past-paper practice." },
              { q: "How do I pay?", a: "Securely by card or bank transfer via our payment gateway. Funds sit in escrow and are released only when lessons are delivered." },
              { q: "Can I cancel or reschedule?", a: "Yes. Rescheduling is free within your package window. Cancellations follow our published policy and unused escrow balances are refundable per policy." },
            ].map((f) => (
              <details key={f.q} className="rounded-xl border border-ink-200 bg-white px-5 py-4">
