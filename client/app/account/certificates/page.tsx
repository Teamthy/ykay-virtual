    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-brand-navy flex items-center gap-2">
          <Award className="text-brand-gold" /> My certificates
        </h1>
      ...
      {certs.isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {(certs.data ?? []).map((c) => (
            <div
              key={c.id}
              className="rounded-2xl border border-brand-gold/40 bg-gradient-to-br from-white to-brand-gold/5 p-6 shadow-soft"
            >
              <div className="flex items-start justify-between">
                <span className="grid h-10 w-10 place-items-center rounded-full bg-brand-navy text-brand-gold">
                  <Award size={18} />
                </span>
              ...
              <h2 className="mt-3 font-display text-lg font-bold text-brand-navy">{c.title}</h2>
