  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#0F2A1A]/65 text-sm mt-1">Review enrolment applications: offer, accept or reject.</p>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {rows.map((a) => (
            <div key={a.id} className="rounded-2xl border border-black/10 bg-white p-5 shadow-soft">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-bold text-[#0F2A1A]">{a.applicant_name || "Application"}</h3>
              ...
              {a.notes && <p className="mt-3 text-sm text-[#0F2A1A]/70 line-clamp-2">{a.notes}</p>}
              {a.status === "OFFERED" && a.offer_fee && (
                <p className="mt-2 text-sm font-bold text-[#0F2A1A]">
                  Offer fee: {a.offer_fee.toLocaleString()} {a.offer_currency || "NGN"}
                </p>
              )}
              <p className="mt-3 text-[11px] text-[#0F2A1A]/65">Submitted {new Date(a.created_at).toLocaleDateString()}</p>
    ...
    <div>
      ...
      {open && (
        <div className="mt-2 space-y-1.5">
          {(docs.data ?? []).map((d) => (
            <a key={d.id} href={d.url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm text-[#0F2A1A] hover:underline">
              <FileText size={14} /> {d.name}
            </a>
          ))}
          {docs.data && docs.data.length === 0 && <p className="text-xs text-[#0F2A1A]/65">No documents uploaded.</p>}
        </div>
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold text-[#0F2A1A]">Offer a place</h2>
          <button onClick={onClose} className="text-[#0F2A1A]/65 hover:text-[#0F2A1A]/70"><X size={18} /></button>
        </div>
        <label className="block text-sm">
          <span className="font-medium text-[#0F2A1A]/75">Offer fee (optional)</span>
          <input value={fee} onChange={(e) => setFee(e.target.value)} type="number" min={0} placeholder="e.g. 50000" className="mt-1 w-full rounded-xl border border-black/10 px-4 py-2.5 text-sm focus:border-[#D6FF57] focus:outline-none" />
        </label>
        <label className="mt-3 block text-sm">
          <span className="font-medium text-[#0F2A1A]/75">Currency</span>
          <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="mt-1 w-full rounded-xl border border-black/10 px-4 py-2.5 text-sm">
            {["NGN", "USD", "GBP"].map((c) => <option key={c}>{c}</option>)}
          </select>
        </label>
        <label className="mt-3 block text-sm">
          <span className="font-medium text-[#0F2A1A]/75">Message (optional)</span>
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={2} className="mt-1 w-full rounded-xl border border-black/10 px-4 py-2.5 text-sm focus:border-[#D6FF57] focus:outline-none" />
        </label>
