...
export default function AdminChatPage() {
  ...
  const trends = useQuery({
    queryKey: ["admin", "chat", "trends"],
    queryFn: () => getChatTrends(14),
  });
  ...
  const activeThread: ChatThread | undefined = threads.data?.find(
    (t) => t.id === activeId,
  );
  const escalatedCount = (threads.data ?? []).filter(
  ...
  const stats = [
    ...
    {
      label: "Avg rating",
      value: analytics.data?.avg_rating
        ? `★ ${analytics.data.avg_rating.toFixed(1)}`
        : "-",
    },
    {
      label: "CSAT (4★+)",
      value: analytics.data ? `${Math.round(analytics.data.csat)}%` : "-",
    },
    {
      label: "Deflection",
      value: analytics.data
        ? `${Math.round(analytics.data.deflection_rate * 100)}%`
        : "-",
    },
              ...
              {(threads.data ?? []).map((t) => (
                ...
                >
                  ...
                  <span className="mt-0.5 block text-[11px] text-ink-400">
                    ...
                    {t.rating
                      ? ` · ★${t.rating}${t.rating_comment ? " - " + t.rating_comment.slice(0, 24) : ""}`
                      : ""}
                  </span>
                </button>
              ))}
