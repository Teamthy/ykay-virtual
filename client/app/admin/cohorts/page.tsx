...
import {
  createAdminCohort,
  listAdminCohorts,
  setAdminCohortStatus,
  listApprovedTutors,
  assignAdminCohortTutor,
  listCohortJoins,
  reviewCohortJoin,
  listPendingEnrollments,
  updateCohort,
  type AdminCohort,
  type AdminVettingProfile,
...
// Admin cohort manager (working-doc §12) - YK-Virtual design system:
...
export default function AdminCohortsPage() {
  ...
  const reviewJoinMut = useMutation({
    ...
    }) => reviewCohortJoin(id, status),
    onSuccess: (_d, { status }) => {
      toast.success(
        status === "APPROVED"
          ? "Tutor assigned to cohort"
  ...
  const setStatusMut = useMutation({
    mutationFn: ({ id, s }: { id: string; s: string }) =>
      setAdminCohortStatus(id, s),
    onSuccess: (_d, { s }) => {
      toast.success(
        s === "PUBLISHED" ? "Cohort published" : `Cohort ${s.toLowerCase()}`,
      );
      qc.invalidateQueries({ queryKey: ["admin", "cohorts"] });
    },
  ...
  const assignTutorMut = useMutation({
    mutationFn: ({ id, tutorId }: { id: string; tutorId: string }) =>
      assignAdminCohortTutor(id, tutorId),
    onSuccess: () => {
    ...
    onError: () =>
      toast.error("Could not assign tutor (tutor must be approved)"),
  });
  ...
  const columns: Column<AdminCohort>[] = [
      ...
      cell: (c) => (
        <div className="w-28">
          <Progress
            value={
              c.capacity
                ? Math.min((c.enrolled_count / c.capacity) * 100, 100)
                : 0
            }
            size="sm"
            showValue={false}
    ...
    },
    {
      ...
      cell: (c) => {
        ...
        const name =
          tutors.find((t) => t.id === draftId)?.display_name ?? "Unassigned";
        ...
        return (
          <div className="flex flex-col gap-1.5 w-56">
            <span className="inline-flex items-center gap-1 text-[11px] font-semibold">
              <UserCheck
                size={12}
                className={isAwaiting ? "text-amber-500" : "text-emerald-600"}
              />
            ...
            <div className="flex gap-1.5">
              ...
              <Button
                ...
                onClick={() =>
                  assignTutorMut.mutate({ id: c.id, tutorId: draftId })
                }
              >
      ...
      cell: (c) => (
        <div className="flex justify-end gap-2">
          ...
          {c.status === "DRAFT" && (
            <Button
              size="sm"
              onClick={() => setStatusMut.mutate({ id: c.id, s: "PUBLISHED" })}
            >
              Publish
            </Button>
          )}
          {c.status === "PUBLISHED" && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => setStatusMut.mutate({ id: c.id, s: "CANCELLED" })}
            >
              Cancel
            </Button>
          )}
          {c.status === "FULL" && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => setStatusMut.mutate({ id: c.id, s: "ONGOING" })}
            >
              Start
            </Button>
          )}
          {c.status === "ONGOING" && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => setStatusMut.mutate({ id: c.id, s: "COMPLETED" })}
            >
              Complete
            </Button>
          )}
          {c.status === "CANCELLED" && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => setStatusMut.mutate({ id: c.id, s: "DRAFT" })}
            >
              Restore
            </Button>
          )}
    ...
    <div className="space-y-6">
            ...
            {(joins.data ?? []).map((j) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    disabled={reviewJoinMut.isPending}
                    onClick={() =>
                      reviewJoinMut.mutate({ id: j.id, status: "APPROVED" })
                    }
                  >
                    Approve
                  </Button>
                  <Button
                    ...
                    disabled={reviewJoinMut.isPending}
                    onClick={() =>
                      reviewJoinMut.mutate({ id: j.id, status: "REJECTED" })
                    }
                  >
                    Reject
                  </Button>
      ...
      <DataTable
        ...
        empty={{
          ...
          description:
            "Create your first cohort and it will appear here with capacity and status.",
        }}
      />
        ...
        if (!res.ok) {
          ...
          throw new Error(
            errBody?.error?.message ||
              "Banner upload failed — cohort was created without an image",
          );
        }
      }
  ...
  return (
    <div className="rounded-2xl border border-ink-100 bg-white p-6 space-y-4 shadow-soft">
      ...
      <div className="flex gap-3">
        <Button variant="gold" disabled={busy} onClick={() => void submit()}>
          {busy ? "Creating…" : "Create draft cohort"}
        </Button>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs font-bold text-ink-600">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, enrollment_opens_at: e.target.value })
              }
              className={field}
            />
          </label>
          <label className="text-xs font-bold text-ink-600">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, enrollment_closes_at: e.target.value })
              }
              className={field}
            />
          </label>
