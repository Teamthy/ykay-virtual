  ...
  if (q.error || !q.data) {
    return (
      <div className="p-8">
        ...
        <Link
          href="/admin/payments"
          className="mt-2 inline-block text-sm font-bold text-primary-dark"
        >
          Back to payments
        </Link>
      </div>
  ...
  return (
    <div className="space-y-6 px-4 py-6 md:px-8">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-400">
        <Link href="/admin/payments" className="hover:text-primary-dark">
          Payments
        </Link>{" "}
        / {order.order_number}
      </p>
      ...
      <dl className="grid gap-4 rounded-2xl border border-ink-100 bg-white p-5 sm:grid-cols-3">
        <div>
          <dt className="text-[11px] font-bold uppercase text-ink-400">
            Amount
          </dt>
        ...
        <div>
          ...
          {payer?.email && (
            <dd className="text-xs text-ink-500">{payer.email}</dd>
          )}
          {payer?.phone && (
            <dd className="text-xs text-ink-500">{payer.phone}</dd>
          )}
        </div>
        <div>
          <dt className="text-[11px] font-bold uppercase text-ink-400">
          ...
          {student?.level && (
            <dd className="text-xs text-ink-500">{student.level}</dd>
          )}
          {student?.school && (
            <dd className="text-xs text-ink-500">{student.school}</dd>
          )}
        </div>
      </dl>
      ...
      <section className="rounded-2xl border border-ink-100 bg-white p-5">
        ...
        <ul className="mt-3 divide-y divide-ink-50">
          ...
          {(items ?? []).length === 0 && (
            <li className="py-4 text-sm text-ink-400">
