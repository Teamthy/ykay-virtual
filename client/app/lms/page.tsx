...
// Student LMS hub â€” my courses, attendance, assignments, quizzes, reports.
    ...
    <main className="pb-16">
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-500">
                {user ? `Signed in as ${user.email}` : "Student portal"} â€” courses, assignments, quizzes and progress.
              </p>
            </div>
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <Stat label="Active courses" value={courses.length || "â€”"} hint="Cohorts you're enrolled in" />
          <Stat label="Attendance" value={attendance.data ? `${attendance.data.rate}%` : "â€”"} hint="Across tracked lessons" />
              ...
              {(assignments.data ?? []).map((a) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {sub?.score !== undefined ? `${sub.score}/${a.max_score ?? "â€”"} graded` : sub ? "Submitted" : "Pending"}
                    </span>
                  </div>
                );
              })}
        ...
        <Section title="Progress reports">
          {(reports.data ?? []).length === 0 ? (
            <p className="rounded-2xl border border-dashed border-ink-200 bg-white p-6 text-center text-sm text-ink-500">
              No progress reports yet â€” your tutor will share them here.
            </p>
          ) : (
            <div className="grid gap-4 md:grid-cols-2">
              {(reports.data ?? []).map((r) => (
