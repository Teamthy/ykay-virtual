        ...
        {saved.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {saved.map((t) => (
              <div
                key={t.slug}
                className="flex items-center gap-4 rounded-2xl border border-ink-100 bg-white p-4 shadow-sm dark:border-ink-700 dark:bg-[#141C2E]"
              >
                ...
                <div className="min-w-0 flex-1">
                  <Link
                    href={`/tutors/${t.slug}`}
                    className="font-bold text-brand-navy hover:text-brand-gold-dark dark:text-white"
                  >
                    {t.name}
                  </Link>
