...
const POINTS = [
  ...
  { icon: <GraduationCap size={18} />, title: "Online and in-person", body: "Teach from home or visit families across Lagos and other Nigerian cities." },
  { icon: <Users size={18} />, title: "A team behind you", body: "Vetting, matching and support so you can focus on teaching." },
];
    ...
    <section className="py-16 bg-surface-muted">
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 text-center">
        ...
        <div className="mt-10 grid sm:grid-cols-3 gap-6">
          {POINTS.map((s) => (
            <div key={s.title} className="rounded-2xl border border-ink-100 bg-white p-7 shadow-soft">
              <div className="mx-auto grid h-11 w-11 place-items-center rounded-xl bg-brand-blue-light text-brand-blue">
                {s.icon}
              </div>
              <p className="mt-4 text-lg font-extrabold tracking-tight text-brand-navy">{s.title}</p>
              <p className="mt-1 text-sm font-semibold text-ink-500">{s.body}</p>
            </div>
          ))}
        </div>
