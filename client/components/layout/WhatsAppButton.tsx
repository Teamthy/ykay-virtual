...
const DEFAULT_PREFILL =
  "Hello NUVORA! I have a question about your courses and enrolment.";

export function useWhatsAppContact() {
  const [contact, setContact] = useState<{ number: string; link: string } | null>(null);
  useEffect(() => {
    let cancelled = false;
    apiFetch<{ whatsapp_number: string; whatsapp_link: string }>("/site/contact")
      .then((res) => {
        if (cancelled) return;
        if (res?.data?.whatsapp_link) {
          setContact({ number: res.data.whatsapp_number, link: res.data.whatsapp_link });
        }
      })
      .catch(() => {
...
export function WhatsAppHomeCard({ prefill = DEFAULT_PREFILL }: { prefill?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1">
        ...
        <span className="block text-xs text-ink-500">Live chat with our team — payments, enrolment, help</span>
      </span>
      <ChevronRightIcon />
    </a>
  );
...
function ChevronRightIcon() {
  return (
    <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-ink-400" aria-hidden="true">
