...
export function LibraryCard({ item }: { item: LibraryItem }) {
  ...
  return (
    <Link
      href={href}
      className="group overflow-hidden rounded-2xl border border-black/10 bg-white shadow-soft transition-shadow hover:shadow-card"
    >
      <div className="relative h-36 w-full overflow-hidden bg-[#0F2A1A]">
        {item.thumbnail_url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={item.thumbnail_url} alt="" className="h-full w-full object-cover" loading="lazy" />
        ) : (
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-[#0F2A1A] via-[#0F2A1A] to-ink-700">
            <Video className="text-white/65" size={40} />
          </div>
        )}
        <div className="absolute inset-0 flex items-center justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
          <span
            className={`flex h-12 w-12 items-center justify-center rounded-full ${
              locked ? "bg-ink-700/80 text-white" : "bg-[#D6FF57] text-[#0F2A1A]"
            }`}
          >
        ...
        {item.featured && (
          <span className="absolute left-3 top-3 rounded-full bg-[#D6FF57] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#0F2A1A]">
            Featured
          </span>
        )}
      ...
      <div className="space-y-1.5 p-4">
        <h3 className="line-clamp-1 text-sm font-bold text-[#0F2A1A]">{item.title}</h3>
        {item.programme_title && (
          <p className="line-clamp-1 text-xs text-[#0F2A1A]/65">{item.programme_title}</p>
        )}
        {item.subjects.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-0.5">
            {item.subjects.slice(0, 3).map((s) => (
              <span key={s} className="rounded-full bg-[#F9F6ED] px-2 py-0.5 text-[10px] font-medium text-[#0F2A1A]/70">
                {s}
              </span>
            ))}
          </div>
