...
test("public catalogue renders seeded tutor and profile", async ({ page }) => {
  ...
  const card = page
    .locator("div")
    .filter({ has: page.getByRole("link", { name: "message", exact: true }) })
    .first();
  await expect(card.getByText(/@/)).toHaveCount(0);

  // Inner-page hero uses the grid template (eyebrow pill + headline).
  await expect(
    page.getByRole("heading", { name: "Find your perfect tutor" }),
  ).toBeVisible();
...
test("home page: batch-2 sections present, healthcare gone, exam cards link", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(page.getByText("We do home tutoring the right way")).toHaveCount(
    0,
  );
...
test("parent pilot journey: register → learner → booking → webhook → LMS", async ({
  page,
  request,
}) => {
  ...
  const wh = await request.post(`${API}/payments/webhooks/PAYSTACK`, {
    ...
    headers: {
      ...
      "X-Paystack-Signature": hmac512(payload),
    },
  });
...
test("cross-family isolation: parent B cannot see parent A's learner", async ({
  request,
}) => {
  ...
  const tamper = await request.get(
    `${API}/me/lessons?student_profile_id=${sidA}`,
  );
...
test("student role cannot reach admin surfaces", async ({ page, request }) => {
  ...
  await expect(
    page.getByText(/Hi|Welcome|Kemi|dashboard/i).first(),
  ).toBeVisible();
});

test("first-time wizard: 3 steps then the role dashboard", async ({
  page,
  request,
}) => {
...
test("become-a-tutor: marketing page + apply step creates the vetting profile", async ({
  page,
  request,
}) => {
  ...
  await page
    .getByLabel("Bio")
    .fill("Five years teaching WAEC and IGCSE maths.");
  await page
    .getByRole("button", { name: /Create profile & continue/ })
    .click({ noWaitAfter: true });
  await expect(page).toHaveURL(/become-tutor\/subjects/, { timeout: 20_000 });
  await expect(page.getByText(/subjects/i).first()).toBeVisible();
});

test("page sweep: every public surface renders (no application errors)", async ({
  page,
}) => {
  const routes = [
    "/",
