...
const profileSchema = z.object({
  ...
  headline: z
    .string()
    .min(5, "A short headline helps parents understand your style"),
  bio: z.string().min(30, "Tell parents a little more (min 30 characters)"),
  years_experience: z.coerce
    .number()
    .min(1, "At least 1 year of teaching experience"),
...
export function OnboardingStepper({ current }: { current: number }) {
  ...
  // YK-Virtual design system tokens.
  return (
    <Stepper
      steps={["Profile", "Subjects", "Documents", "Quiz", "In review"]}
      current={current}
      className="mb-8"
    />
  );
...
export function DocumentsStep({
  ...
  const upload = async () => {
    if (files.length === 0) {
      setError("Choose at least one ID document first");
      return;
    }
    setUploading(true);
    setError(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal
        ...
        description="Your ID documents are submitted to the YK-Virtual review team."
        footer={
...
export function AssessmentStep({
  ...
  const [attempt, setAttempt] = useState<Awaited<
    ReturnType<typeof startAssessment>
  > | null>(null);
  const [answers, setAnswers] = useState<Record<string, number>>({});
  const [result, setResult] = useState<Awaited<
    ReturnType<typeof submitAssessment>
  > | null>(null);
  ...
  if (result) {
    return (
      <div
        className={`border rounded-2xl p-8 text-center space-y-3 ${result.passed ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}
      >
        ...
        <p className="text-sm text-ink-700">
          ...
          {result.passed
            ? " - competency valid for 12 months."
            : " - you can retake once your profile is under review."}
        </p>
    ...
    return (
      <div className="border rounded-2xl p-6 space-y-5">
        ...
        <Button
          ...
          className="w-full"
          disabled={
            busy || Object.keys(answers).length < attempt.questions.length
          }
          onClick={submit}
        >
...
export function SubmittedState({ profile }: { profile: TutorProfile }) {
  const statusCopy: Record<string, { label: string; hint: string }> = {
    DRAFT: {
      label: "Draft",
      hint: "Finish your application to submit for review.",
    },
    SUBMITTED: {
      label: "Submitted",
      hint: "Our team will review your application shortly.",
    },
    UNDER_REVIEW: {
      label: "Under review",
      hint: "We are checking your credentials and documents.",
    },
    INTERVIEW: {
      label: "Interview scheduled",
      hint: "Our team will reach out to schedule your interview.",
    },
    VERIFICATION: {
      label: "Verification",
      hint: "Final identity checks in progress.",
    },
    APPROVED: {
      label: "Approved",
      hint: "You are live on the marketplace. Bookings can come in now.",
    },
    REJECTED: {
      label: "Not approved",
      hint: "Our team will contact you with next steps.",
    },
    HOLD: {
      label: "On hold",
      hint: "Your application is paused - we will be in touch.",
    },
