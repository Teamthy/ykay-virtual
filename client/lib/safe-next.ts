...
export function safeNextPath(raw: string | null | undefined): string | null {
  ...
  if (v.startsWith("//") || v.includes("\\") || v.includes("://") || v.includes("%5c")) return null;
  // eslint-disable-next-line no-control-regex
  if (/[\u0000-\u001f]/.test(v)) return null;
  let path: string;
  let search: string;
  try {
    const u = new URL(v, "https://nuvora.invalid");
    path = u.pathname;
    search = u.search;
  } catch {
    return null;
  ...
  if (AUTH_PREFIXES.some((p) => lower === p || lower.startsWith(`${p}/`) || lower.startsWith(`${p}?`))) {
    return null;
  }
  return `${path}${search}`;
}
