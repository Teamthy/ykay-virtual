#!/usr/bin/env bash
# YK-Virtual — health-gated production deploy on OCI with automatic rollback.
#
...
echo "→ deploying ${IMAGE_TAG} (current: ${PREV_TAG:-none})"
...
echo "→ waiting for api to become healthy (max 150s)…"
deadline=$(( $(date +%s) + 150 ))
healthy=0
...
if [[ $ready -ne 1 ]]; then
  echo "✗ deployment failed health gate. Recent logs:" >&2
  ...
  if [[ -n "${PREV_TAG:-}" ]]; then
    ...
    if [[ "$(docker inspect --format='{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' ykv-prod-api-1 2>/dev/null)" == "healthy" ]]; then
      echo "✓ rolled back to ${PREV_TAG} (investigate ${IMAGE_TAG})" >&2
    else
...
echo "✓ ${IMAGE_TAG} live and healthy"
