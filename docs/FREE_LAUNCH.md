# NUVORA â€” free launch (â‚¦50â€“70k, including domain)
...
**Buy:** `nuvora.com.ng` (~â‚¦6â€“8k/yr) or `nuvora.ng` (~â‚¦13â€“15k). `.com` is
~â‚¦18â€“22k.
...
3. **New project** â†’ name `nuvora` â†’ region closest to Europe/US East
...
| `EMAIL_FROM` | `NUVORA <beth.t@example.com>` | Â§7 |
...
3. Redeploy.  
4. **Settings â†’ Domains** â†’ add `nuvora.com.ng` â†’ copy the DNS records to
   the registrar.
...
5. **After domain:** Domains â†’ Add `nuvora.com.ng` â†’ add the DNS TXT/MX/CNAME
   they list â†’ then `EMAIL_FROM=NUVORA <leo.a@example.org>`.
...
1. [https://console.cloud.google.com](https://console.cloud.google.com) â†’
   New project `nuvora`.  
2. **APIs & Services â†’ OAuth consent screen** â†’ External â†’ app name NUVORA.  
...
5. Create a lesson â†’ meeting URL should look like `https://meet.jit.si/nuvora-â€¦`.
