...
**Gaps found in MVP:**
- âŒ **Private tuition end-to-end booking/payment** is the weakest link — a request form exists but a full private-package purchase flow (price → package → schedule → pay → activate) is not a complete first-class journey.
...
## 3. Phase 2 / Future
- âš ï¸ Automated assessments/question banks — assessment create/start/submit exists; full question-bank auto-marking is partial.
- ✅ Tutor payouts/wallet — payouts + escrow implemented (Phase 2 items already built).
- âŒ Recorded lesson library — video upload/offline exists but no full recorded-library CMS.
- âŒ Certificates — not implemented.
- âš ï¸ Referral engine — implemented (not coupon).
- âš ï¸ Advanced analytics — funnel/fill/revenue present.
- âš ï¸ Institutional/school accounts — institutions list exists; full B2B workflow partial.
- 🚧 **Virtual school build started** — see `docs/VIRTUAL_SCHOOL_PLAN.md`. Pillar 1 (academic sessions & terms) backend shipped 2026-08-25 (migration 000063, admin + public API, `cohorts.term_id` anchor).
- ✅ Term structure — sessions/terms with lifecycles, overlap guards, one-ACTIVE-per-scope, enrolment windows (admin UI pending).
- âŒ Timetable/subject registration, homeroom/pastoral, transcripts, assemblies, accreditation — **not implemented** (Pillars 2–7 in the plan). Formal admissions exists at âš ï¸ (applications + review; no document uploads/fees wiring yet).
...
- âš ï¸ **Exam prep** — now includes IGCSE, WAEC, NECO, JAMB, A-Level, **SAT** (added). GCE is covered by A-Level ("GCE Advanced Level").
...
| FR-10 No double-booking | âš ï¸ partial — needs explicit conflict check on lessons |
...
- âŒ **Certificate** entity — missing (future).
...
- âš ï¸ Institutional admin scoping — declared but full institution-scoped permission enforcement is partial.
...
- âš ï¸ **MFA for admins** — not implemented (recommended by working doc).
- âš ï¸ **Malware scanning** for uploads — size/MIME validated; scanning strategy not implemented.
...
- âš ï¸ AC-05 no double-booking — needs explicit overlap guard.
- âœ… AC-06 idempotent payment webhook.
- âš ï¸ AC-07 reschedule updates dashboards — partial.
