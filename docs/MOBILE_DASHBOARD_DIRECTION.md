# YK-Virtual Mobile — Dashboard Design Direction

> Reinterpretation of two reference dashboards (a light health-style app and
> a deep-green finance-style app) for YK-Virtual's own product, users and data.
...
YK-Virtual keeps its own tokens (lime `#70F250` / deep `#013920`, Anton + DM
Sans, radius 8/12/16/20/pill) — the references happen to share the green
family, which is YK-Virtual's existing brand, not a new import.
...
| Home · signed out     | Welcome card — what YK-Virtual is, Log in / Create account                                                            |
...
glyph). This is the reference pattern applied to YK-Virtual's actual topology:
