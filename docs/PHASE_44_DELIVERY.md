...
- All collectors namespaced nuvora_*, registered on the default registry
...
- HTTP: nuvora_http_requests_total{method,route,code} +
  nuvora_http_request_duration_seconds{method,route} histograms — middleware
...
- Jobs: enqueued/completed/retried/dead-lettered/dropped counters
  {type,backend=redis|memory} + nuvora_queue_depth{backend,state} gauges —
...
- Worker: nuvora_worker_cron_runs_total{cron,result} +
  nuvora_worker_cron_last_success_timestamp{cron} stamped on every cron run
  (boot recovery + 15m/7d/24h ticks) — the staleness-alert source.
- nuvora_build_info{version} stamped at API boot.
...
- deploy/grafana/dashboards/nuvora-api.json — 12-panel production dashboard
...
inspect) → heartbeat textfile (nuvora_dr_drill.prom) for the
NuvoraDrillOverdue alert. Exit codes: 0 pass / 1 fail (stale, missing,
mismatch all verified).

### scripts/backup.sh
Writes nuvora_backup.prom (success ts + size) and
nuvora_backup.prom.fail (failure ts, removed on success) to
