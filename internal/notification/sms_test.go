...
func TestTermiiSendBuildsCorrectRequest(t *testing.T) {
	...
	s := NewTermiiSMSSender("key-123", "Nuvora", "")
	s.BaseURL = srv.URL
	...
	if got.From != "Nuvora" || got.APIKey != "key-123" || got.Channel != "dnd" || got.Type != "plain" {
...
func TestTermiiRejectsGatewayError(t *testing.T) {
	...
	s := NewTermiiSMSSender("key", "Nuvora", "")
	s.BaseURL = srv.URL
...
func TestTermiiHTTPError(t *testing.T) {
	...
	s := NewTermiiSMSSender("bad", "Nuvora", "")
	s.BaseURL = srv.URL
