...
func NewRouterWithOrigins(version string, handlers *Handlers, allowedOrigins string, sessionAuth func(http.Handler) http.Handler, readyCheck func() error, blockFrames bool) *Router {
	...
	// credential endpoints â€” the token is verified against the College portal
	...
	// grading â€” per-student variation the fixed tutor papers can't offer.
	mux.HandleFunc("GET "+v1+"/cbt/subjects", handlers.CBTBank.ListSubjects)
	mux.HandleFunc("GET "+v1+"/cbt/subjects/{slug}/topics", handlers.CBTBank.ListTopics)
	...
	// anonymous-cacheable (metadata only â€” the service strips video/transcript
