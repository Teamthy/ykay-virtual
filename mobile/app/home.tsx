...
// Home — the YK-Virtual command center (design direction: docs/MOBILE_DASHBOARD_DIRECTION.md).
...
export default function Home() {
    ...
    try {
      ...
      if (isTutor) {
        ...
        const [ls, prog, at] = await Promise.all([
          apiFetch<Lesson[]>(`/me/lessons${learnerQuery(learnerId)}`).catch(
            () => ({ data: [] as Lesson[] }),
          ),
  ...
  const isTutor = roles.includes("TUTOR");
  const isAdmin =
    roles.includes("ACADEMIC_ADMIN") || roles.includes("SUPER_ADMIN");
  ...
  const avgScore =
    submitted.length > 0
      ? Math.round(
          submitted.reduce((n, a) => n + (a.score ?? 0), 0) / submitted.length,
        )
      : null;
  ...
  const nextToday = useMemo(
    () =>
      todayLessons.find((l) => new Date(l.start_at).getTime() >= Date.now()) ??
      todayLessons[0],
    [todayLessons],
  );
      ...
      <Screen scroll>
        ...
        <Animated.View entering={FadeIn.delay(60).duration(240)}>
          <View style={styles.header}>
            ...
            <View style={{ flex: 1, marginLeft: spacing.sm }}>
              ...
              <AppText variant="h2">
                {signedOut ? "YK-Virtual" : firstName || "there"}
              </AppText>
            </View>
            <Pressable
            ...
            >
              ...
              {unread > 0 && (
                <View
                  style={[styles.bellBadge, { backgroundColor: colors.green }]}
                >
                  <AppText
                    variant="caption"
                    style={{ color: colors.ink[950], fontWeight: "800" }}
                  >
                    {unread > 9 ? "9+" : unread}
                  </AppText>
                </View>
        ...
        <Animated.View entering={FadeIn.delay(120).duration(240)}>
          {signedOut ? (
            <Card style={styles.welcomeCard}>
              ...
              >
                Track lessons, sit timed CBT practice exams, message vetted
                tutors and manage tuition escrow — all in one place.
              </AppText>
              <Button
                label="Log in"
                full
                ...
                <>
                  ...
                  <View style={styles.heroSubRow}>
                    {nextToday ? (
                      <>
                        ...
                        <View
                          style={[
                            styles.heroChip,
                            {
                              backgroundColor: nextToday.meeting_url
                                ? "rgba(112,242,80,0.18)"
                        ...
                        <AppText style={styles.heroCap}>
                          {todayLessons.length}{" "}
                          {todayLessons.length === 1 ? "class" : "classes"}{" "}
                          today
                        </AppText>
                      </>
                    ) : (
                      <AppText style={styles.heroCap}>
                        {nextUpcoming
                          ? `Next: ${fmtDay(nextUpcoming.start_at)} · ${fmtTime(nextUpcoming.start_at)}`
                          : "Your week is clear — relax or prep your exams."}
                      </AppText>
                    )}
                  </View>
                ...
                <>
                    ...
                    <View style={styles.heroSubRow}>
                      ...
                      <View
                        style={[
                          styles.heroChip,
                          {
                            backgroundColor: next.meeting_url
                              ? "rgba(112,242,80,0.18)"
                              : "rgba(255,255,255,0.12)",
                          },
                        ]}
                  ...
                  <View style={styles.heroActions}>
                    <Pressable
                      accessibilityRole="button"
                      onPress={() =>
                        router.push((next ? "/lms" : "/subjects") as never)
                      }
                      style={[
                        styles.heroCta,
                        { backgroundColor: colors.green },
          ...
          >
            {(isTutor
              ? [
                  ...
                  {
                    label: "HELD",
                    value: earnings ? formatNaira(earnings.held_total) : "₦0",
                    href: "/tutor/earnings",
                  },
                  {
                    label: "PAID OUT",
                    value: earnings ? formatNaira(earnings.paid_total) : "₦0",
                    href: "/tutor/earnings",
                  },
                ]
              : [
              ...
              >
                <AppText
                  variant="caption"
                  style={{ color: colors.ink[400], letterSpacing: 0.8 }}
                >
                  {m.label}
                </AppText>
                <AppText
                  variant="h2"
                  style={{ color: colors.deep, marginTop: 4 }}
                  numberOfLines={1}
        ...
        {!signedOut && (
          <Animated.View entering={FadeIn.delay(200).duration(240)}>
            <AppText
              variant="label"
              style={[styles.section, { color: colors.ink[500] }]}
            >
              QUICK ACTIONS
            </AppText>
            {isTutor ? (
              ...
              <Button
                label={isParent ? "Find a tutor" : "Take a practice exam"}
                full
                onPress={() =>
                  router.push((isParent ? "/search" : "/practice") as never)
                }
              />
            )}
          ...
          {signedOut ? (
            ...
            <View style={styles.activity}>
              {upcoming.slice(0, 2).map((l) => (
                <Card
                  key={l.id}
                  onPress={() => router.push("/lms" as never)}
                  style={styles.activityRow}
                >
                  <View
                    style={[
              ...
              {submitted.slice(0, 2).map((a) => (
                ...
                >
                  ...
                  <View style={{ flex: 1, marginLeft: spacing.sm }}>
                    ...
                    >
                      {a.exam_subject} ·{" "}
                      {a.submitted_at ? fmtDay(a.submitted_at) : ""}
                    </AppText>
                  </View>
                  <View
                    style={[
