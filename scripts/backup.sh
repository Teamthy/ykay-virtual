...
#   nuvora_backup.prom       ← success heartbeat + size in bytes
#   nuvora_backup.prom.fail  ← failure heartbeat (deleted on success)
# NuvoraBackupStale in deploy/prometheus/alerts.yml fires on a stale heartbeat.
set -euo pipefail
...
fail_metric() {
  if [ -n "${BACKUP_METRICS_DIR:-}" ]; then
    ...
    printf "nuvora_backup_last_failure_timestamp %s\n" "$(date +%s)" > "$BACKUP_METRICS_DIR/nuvora_backup.prom.fail"
  fi
}
...
if [ -n "${BACKUP_METRICS_DIR:-}" ]; then
  mkdir -p "$BACKUP_METRICS_DIR"
  printf "nuvora_backup_last_success_timestamp %s\nnuvora_backup_size_bytes %s\n" \
    "$(date +%s)" "$(stat -c%s "$OUT" 2>/dev/null || stat -f%z "$OUT")" > "$BACKUP_METRICS_DIR/nuvora_backup.prom"
  rm -f "$BACKUP_METRICS_DIR/nuvora_backup.prom.fail"
